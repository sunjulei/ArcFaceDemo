...
public class InfoManage extends Activity {
   ...
   private OutaccountDAO outaccountDAO = new OutaccountDAO(InfoManage.this);// 创建OutaccountDAO对象
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.xzzc1_xg);// 设置布局文件
        tvtitle = (TextView) findViewById(R.id.title);// 获取标题标签对象
        textView = (TextView) findViewById(R.id.tv4);// 获取地点/付款方标签对象
        txtMoney = (EditText) findViewById(R.id.ed1);// 获取金额文本框
        txtTime = (EditText) findViewById(R.id.ed2);// 获取时间文本框
        spType = (Spinner) findViewById(R.id.SP3);// 获取类别下拉列表
        txtHA = (EditText) findViewById(R.id.ed4);// 获取地点/付款方文本框
        txtMark = (EditText) findViewById(R.id.ed5);// 获取备注文本框
        btnEdit = (Button) findViewById(R.id.bt1);// 获取修改按钮
        btnDel = (Button) findViewById(R.id.bt2);// 获取删除按钮
        ...
        if (strType.equals("btnoutinfo")) {// 如果类型是btnoutinfo
            ...
            spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> parent, View view,
                                           int pos, long id) {
                    String[] zhichu = getResources().getStringArray(R.array.shouru);
                }
                @Override
                public void onNothingSelected(AdapterView<?> parent) {
                }
