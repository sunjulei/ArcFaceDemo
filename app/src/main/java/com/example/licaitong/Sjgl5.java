...
public class Sjgl5 extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sjgl5);// 设置布局文件

        lvinfo = (ListView) findViewById(R.id.lvinfo);// 获取布局文件中的ListView组件
        btnoutinfo = (Button) findViewById(R.id.btn1);// 获取布局文件中的支出信息按钮
        btnininfo = (Button) findViewById(R.id.btn2);// 获取布局文件中的收入信息按钮
        btnflaginfo = (Button) findViewById(R.id.btn3);// 获取布局文件中的便签信息按钮

        showInfo(R.id.btn1);// 默认显示支出信息

        btnoutinfo.setOnClickListener(new OnClickListener() {// 为支出信息按钮设置监听事件
            @Override
            public void onClick(View arg0) {
                showInfo(R.id.btn1);// 显示支出信息
            }
        });
        ...
        lvinfo.setOnItemClickListener(new OnItemClickListener() {// 为ListView添加项单击事件
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (strType == "btnoutinfo" | strType == "btnininfo") {// 判断如果是支出或者收入信息
                    intent = new Intent(Sjgl5.this, InfoManage.class);// 使用InfoManage窗口初始化Intent对象
                    intent.putExtra(FLAG, new String[] { strid, strType });// 设置要传递的数据
                } else if (strType == "btnflaginfo") {// 判断如果是便签信息
                    intent = new Intent(Sjgl5.this, FlagManage.class);// 使用FlagManage窗口初始化Intent对象
    ...
    private void showInfo(int intType) {// 用来根据传入的管理类型，显示相应的信息
        ...
        switch (intType) {// 以intType为条件进行判断
            case R.id.btn1:// 如果是btnoutinfo按钮
                strType = "btnoutinfo";// 为strType变量赋值
                OutaccountDAO outaccountinfo = new OutaccountDAO(Sjgl5.this);// 创建OutaccountDAO对象
                ...
                break;
            case R.id.btn2:// 如果是btnininfo按钮
                strType = "btnininfo";// 为strType变量赋值
                InaccountDAO inaccountinfo = new InaccountDAO(Sjgl5.this);// 创建InaccountDAO对象
                ...
                break;
            case R.id.btn3:// 如果是btnflaginfo按钮
                strType = "btnflaginfo";// 为strType变量赋值
                FlagDAO flaginfo = new FlagDAO(Sjgl5.this);// 创建FlagDAO对象
